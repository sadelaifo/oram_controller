...
int main(int argc, char *argv[])
{
	...
	NUMCORES = argc - 2;
	ORAM_queue = (list_t **)malloc(sizeof(list_t *) * NUMCORES);
	for (int _i_ = 0; _i_ < NUMCORES; _i_++)
	{
		ORAM_queue[_i_] = list_create();
	}
...
*/
							if (1)
							{
								ORAM_queue_req *new_ORAM_req = init_ORAM_queue_req(addr[numc], READ, instrpc[numc], ROB[numc].tail);
								list_insert(ORAM_queue[numc], (void *)new_ORAM_req);
							}
							else
							{											 // mark this read request as a stash hit
								ROB[numc].comptime[ROB[numc].tail] = -1; //CYCLE_VAL + lat + PIPELINEDEPTH;
							}
						}
						else
						{ /* This must be a 'W'.  We are confirming that while reading the trace. */
							if (opertype[numc] == 'W')
							{
								...
								*/
								if (1)
								{
									ORAM_queue_req *new_ORAM_req = init_ORAM_queue_req(addr[numc], WRITE, instrpc[numc], ROB[numc].tail);
									list_insert(ORAM_queue[numc], (void *)new_ORAM_req);
									//ORAM_queue[numc].push(new_ORAM_req);
									//list_insert(oram_)
								}
								else
								{											 // mark this read request as a stash hit
		...
		for (int _i_ = 0; _i_ < NUMCORES; _i_++)
		{
			if (ORAM_queue[_i_]->count > 0)
			{
				ORAM_queue_req *iter = (ORAM_queue_req *)ORAM_queue[_i_]->head->data; //&ORAM_queue[_i_].front();
				if (iter != NULL)
				{
					// if this ORAM request completion time < current time, then mark it complete in ROB
					if (iter->comptime == -1)
					{
						printf("ERROR, wrong comptime \n");
						return -1;
					}
					if (iter->comptime != 0)
					{
						ROB[numc].comptime[iter->ROB_loc] = iter->comptime;
						if (iter->comptime < CYCLE_VAL)
						{
							ORAM_queue_req *head = (ORAM_queue_req *)list_pop(ORAM_queue[_i_]);
							ORAM_queue_req_destroy(head);
						}
						//ORAM_queue[_i_].pop();
					}
					else
					{
						// if read is not scheduled yet, schedule all read requests
						if (iter->reads_scheduled == 0)
						{
							for (int _j_ = 0; _j_ < ORAM_TREE_LEVEL; _j_++)
							{
								insert_read(iter->vir_address, CYCLE_VAL, _i_, _j_, iter->instrpc);
							}
							iter->reads_scheduled = 1;
						}
						else
						{ // if all reads are scheduled, then check whether we need to schedule writes
							if (iter->read_queue[ORAM_TREE_LEVEL - 1].comptime != 0 &&
								iter->read_queue[ORAM_TREE_LEVEL - 1].comptime < CYCLE_VAL &&
								iter->writes_scheduled == 0)
							{ // if the last read is finished
								for (int _j_ = 0; _j_ < ORAM_TREE_LEVEL; _j_++)
								{
									insert_write(iter->vir_address, CYCLE_VAL, _i_, _j_);
								}
								iter->writes_scheduled = 1;
							}
						}
						if (iter->write_queue[ORAM_TREE_LEVEL - 1].comptime < CYCLE_VAL &&
							iter->write_queue[ORAM_TREE_LEVEL - 1].comptime != 0)
						{
							iter->comptime = iter->write_queue[ORAM_TREE_LEVEL - 1].comptime;
							ROB[_i_].comptime[iter->ROB_loc] = iter->comptime;
						}
					}
				}
			}
