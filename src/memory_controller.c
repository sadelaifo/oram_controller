...
int issue_request_command(request_t *request)
{
	...
	case COL_READ_CMD:
		...
		//auto iter = &ORAM_queue[request->thread_id].front();
		if (ORAM_queue[request->thread_id]->count > 0)
		{
			tmp_iter = (ORAM_queue_req *)ORAM_queue[request->thread_id]->head->data;

			if (request->operation_type == READ)
			{
				tmp_iter->read_queue[request->instruction_id].comptime = request->completion_time + PIPELINEDEPTH;
			}
			else // this request can only be read
			{
				return -1;
			}
		}
	...
	case COL_WRITE_CMD:
		...
		{
			tmp_iter = (ORAM_queue_req *)ORAM_queue[request->thread_id]->head->data;
			if (request->operation_type == WRITE)
			{
				tmp_iter->write_queue[request->instruction_id].comptime = CYCLE_VAL + T_DATA_TRANS + T_WR;
			}
			else
			{
				return -1;
